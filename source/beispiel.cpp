...
int main() {
 ...
 for (std::vector<int>::iterator it = v0.begin(); it != v0.end(); ++it) { // Iteration; Vektor von (0) bis(9); it++ erhoehung um 1; it++=alle elemente
 ...
 std::generate(std::begin(v0), std::end(v0), std::rand); //v0 erhält zufällige Zahlen ("Generate" übergebene Elemente = neuer Wert; dafür genutzte Funktion durch letzten Parameter(rand))
 std::copy( v0.rbegin(), v0.rend(), std::ostream_iterator<int>(std::cout, "\n")); //Iterator Ausgabe der Elemente von v0 in umgekehrter Reihenfolgen (rebegin/rend -reversebegin -reverseend)
